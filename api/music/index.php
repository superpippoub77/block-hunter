...
$musicDir = $rootDir . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'music' . DIRECTORY_SEPARATOR . 'scenes' . DIRECTORY_SEPARATOR . 'game';
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($entries !== false) {
        foreach ($entries as $name) {
            ...
            $list[] = 'assets/music/scenes/game/' . $name;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'rename') {
        ...
        $oldPath = $musicDir . DIRECTORY_SEPARATOR . $oldName;
        $newPath = $musicDir . DIRECTORY_SEPARATOR . $newName;
        ...
        send_json(200, [
            'ok' => true,
            'oldFile' => 'assets/music/scenes/game/' . $oldName,
            'file' => 'assets/music/scenes/game/' . $newName
        ]);
    }
    ...
    $target = $musicDir . DIRECTORY_SEPARATOR . $fileName;
    if (file_put_contents($target, $decoded) === false) {
        send_json(500, ['ok' => false, 'error' => 'cannot write file']);
    }

    send_json(200, ['ok' => true, 'file' => 'assets/music/scenes/game/' . $fileName]);
}
